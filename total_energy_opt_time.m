...
for i = 1 : 1 : 500
    ...
    M = ceil(N * sqrt(c / (c + p_tran(i) * t_proc(i) * log(3))));  % 信息处理最优并行数
    Q_proc(i) = (N + M) * p_tran(i) * log(3) + c * (N + M)^2 / (M * t_proc(i));  % 信息处理能耗
    Q_total(i) = Q_proc(i) + Q_tran(i);  % 总能耗
    if Q_total(i) < Q_total(idx)
        idx = i;
    end
end

percent = (Q_total(500) - Q_total(idx)) / Q_total(500);

figure;
plot(t_tran, Q_total, 'LineWidth', 2, 'Color', [120/255, 183/255, 201/255]);
xlabel('Percentage of information transmission time');
ylabel('Total energy dissipation (kT)');
grid on;
hold on;
plot([t_tran(idx) t_tran(500)], [Q_total(idx) Q_total(500)], '.','MarkerSize',15, 'Color', [229/255, 139/255, 123/255], 'LineWidth',1.5);
